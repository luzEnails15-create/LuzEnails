...
**v8**
- Nuevo: en el panel admin, pestaña **"Citas"**, hay un formulario para
  **agendar una cita exclusiva** — una cita que tú creas manualmente para
  cualquier fecha y hora, incluso si ese día está marcado como "Cerrado"
  en el horario habitual (por ejemplo, un domingo puntual para una
  clienta especial).
  - Esta cita **nunca es visible ni reservable para los clientes**: la
    página pública sigue decidiendo qué días mostrar como disponibles
    únicamente a partir del horario configurado en la pestaña "Horario",
    sin importar si tú agendaste algo ese día. No fue necesario cambiar
    nada de `index.html` ni de las reglas de Firestore para lograr esto.
  - El formulario revisa que la hora elegida no se traslape con otra cita
    ya existente ese mismo día (de cliente o exclusiva) antes de dejarte
    guardarla.
  - En la tabla de citas, las agendadas por ti llevan una etiqueta azul
    **"Exclusiva"** junto al nombre del cliente, para distinguirlas de
    un vistazo.
  - IMPORTANTE: no se requiere volver a pegar `firestore.rules` en
    Firebase; las reglas no cambiaron en esta versión.
