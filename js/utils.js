...
// Combina la configuración guardada en Firestore con los valores por
// defecto, asegurando que "penalty" quede completo aunque el documento
// guardado sea antiguo o le falte algún campo nuevo.
export function mergeBusinessConfig(raw) {
  const defaults = defaultBusinessConfig();
  const merged = { ...defaults, ...(raw || {}) };
  merged.penalty = { ...defaults.penalty, ...((raw && raw.penalty) || {}) };
  return merged;
}
...
// Construye un objeto Date (hora local) a partir de una fecha "YYYY-MM-DD"
// y una hora "HH:MM" tal como se guardan las citas.
export function appointmentDateTime(dateStr, timeStr) {
  const [y, m, d] = dateStr.split("-").map(Number);
  const [hh, mm] = (timeStr || "00:00").split(":").map(Number);
  return new Date(y, m - 1, d, hh, mm);
}

// Horas (puede ser decimal, o negativo si la cita ya pasó) entre ahora y el
// inicio de la cita. Se usa para saber si una cancelación es "tardía".
export function hoursUntil(dateStr, timeStr) {
  const diffMs = appointmentDateTime(dateStr, timeStr).getTime() - Date.now();
  return diffMs / (1000 * 60 * 60);
}

// Suma (o resta) días a una fecha "YYYY-MM-DD" y devuelve otra fecha con el
// mismo formato. Se usa para calcular hasta cuándo queda bloqueada una
// clienta tras acumular cancelaciones tardías seguidas.
export function addDaysToKey(dateStr, days) {
  const [y, m, d] = dateStr.split("-").map(Number);
