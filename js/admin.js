...
import {
  ...
  formatMoney, formatDuration, formatFriendlyDate, colorForIndex, todayKey
} from "./utils.js";
...
function renderScheduleForm() {
  ...
  scheduleForm.slotInterval.value = String(currentConfig.slotInterval || 30);
...
scheduleForm.addEventListener("submit", async (e) => {
  ...
  currentConfig.slotInterval = Number(scheduleForm.slotInterval.value);
...
function renderAppointments() {
  ...
  if (list.length === 0) {
    appointmentsTableBody.innerHTML = `<tr><td colspan="7" class="empty-state">No hay citas para mostrar.</td></tr>`;
    return;
  }
  ...
  appointmentsTableBody.innerHTML = list
    .map(
      ...
      <tr data-id="${a.id}">
        ...
        <td class="row-actions">
          ${a.status === "cancelled" ? "" : '<button data-action="whatsapp">Recordatorio WhatsApp</button>'}
  ...
  appointmentsTableBody.querySelectorAll('[data-action="cancel"]').forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      if (!confirm(`¿Cancelar la cita de ${appt.clientName} el ${formatFriendlyDate(appt.date)} a las ${appt.time}?`)) return;
...
function buildWhatsAppReminderLink(appt) {
  ...
  const message =
    ...
    `el ${formatFriendlyDate(appt.date)} a las ${appt.time} para ${appt.serviceName || "tu servicio"}. ` +
    `Te esperamos.\n\n` +
