...
import {
  WEEKDAY_KEYS, WEEKDAY_LABELS, defaultBusinessConfig, mergeBusinessConfig,
...
const penaltyListMsg = document.getElementById("penaltyListMsg");
...
async function loadConfig() {
  ...
  currentConfig = mergeBusinessConfig(snap.exists() ? snap.data() : null);
...
function renderPenaltyForm() {
  const p = currentConfig.penalty;
  penaltyForm.enabled.checked = p.enabled;
  penaltyForm.lateWindowHours.value = p.lateWindowHours;
  penaltyForm.maxLateCancels.value = p.maxLateCancels;
  penaltyForm.blockDays.value = p.blockDays;
}

penaltyForm.addEventListener("submit", async (e) => {
  e.preventDefault();

  const lateWindowHours = Number(penaltyForm.lateWindowHours.value);
  const maxLateCancels = Number(penaltyForm.maxLateCancels.value);
  const blockDays = Number(penaltyForm.blockDays.value);

  if (!Number.isFinite(lateWindowHours) || lateWindowHours <= 0
    || !Number.isFinite(maxLateCancels) || maxLateCancels < 2
    || !Number.isFinite(blockDays) || blockDays <= 0) {
    showMsg(penaltyMsg, "Revisa que las horas, la cantidad de cancelaciones y los días de bloqueo sean números válidos.", "error");
    return;
  }

  currentConfig.penalty = {
    enabled: penaltyForm.enabled.checked,
    lateWindowHours,
    maxLateCancels,
    blockDays
  };
  ...
  showMsg(penaltyMsg, "Penalización guardada correctamente.", "ok");
});

// ================= CANCELACIONES TARDÍAS =================

async function loadPenalties() {
  const snap = await getDocs(collection(db, "clientPenalties"));
  const list = [];
  snap.forEach((d) => list.push({ id: d.id, ...d.data() }));

  // Solo interesa mostrar clientas con algo que contar: una racha activa o
  // un bloqueo (vigente o pasado, para que quede el historial visible).
  const relevant = list.filter((p) => (p.lateCancelStreak || 0) > 0 || p.blockedUntil);
  relevant.sort((a, b) => (b.blockedUntil || "").localeCompare(a.blockedUntil || ""));

  if (relevant.length === 0) {
    penaltyTableBody.innerHTML = `<tr><td colspan="5" class="empty-state">Ninguna clienta tiene cancelaciones tardías registradas.</td></tr>`;
    return;
  }

  const tKey = todayKey();
  penaltyTableBody.innerHTML = relevant
    .map((p) => {
      const isBlockedNow = p.blockedUntil && p.blockedUntil >= tKey;
      const statusHtml = isBlockedNow
        ? `<span class="pill cancelled">Bloqueada hasta ${formatFriendlyDate(p.blockedUntil)}</span>`
        : (p.blockedUntil ? `<span class="pill">Bloqueo ya venció (${formatFriendlyDate(p.blockedUntil)})</span>` : "—");
      return `
      <tr data-id="${p.id}">
        <td>${escapeHtml(p.phone || p.id)}</td>
        <td>${escapeHtml(p.lastClientName || "")}</td>
        <td>${p.lateCancelStreak || 0}</td>
        <td>${statusHtml}</td>
        <td class="row-actions">
          ${isBlockedNow ? '<button data-action="unblock" class="danger">Quitar bloqueo</button>' : ""}
        </td>
      </tr>`;
    })
    .join("");

  penaltyTableBody.querySelectorAll('[data-action="unblock"]').forEach((btn) => {
    btn.addEventListener("click", async () => {
      const row = btn.closest("tr");
      const id = row.dataset.id;
      if (!confirm("¿Quitar el bloqueo a esta clienta y reiniciar su contador de cancelaciones tardías?")) return;
      await updateDoc(doc(db, "clientPenalties", id), { blockedUntil: null, lateCancelStreak: 0 });
      showMsg(penaltyListMsg, "Bloqueo eliminado.", "ok");
      loadPenalties();
    });
  });
}
