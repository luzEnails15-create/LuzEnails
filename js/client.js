...
import {
  collection, getDocs, getDoc, setDoc, query, where, doc, writeBatch, updateDoc, deleteDoc, serverTimestamp
} from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
import {
  WEEKDAY_KEYS, MONTH_LABELS, WEEKDAY_SHORT,
  defaultBusinessConfig, mergeBusinessConfig, dateKey, todayKey,
  formatMoney, formatDuration, formatFriendlyDate, formatTime12h,
  generateCandidateSlots, overlaps, timeToMinutes, colorForIndex,
  hoursUntil, addDaysToKey
} from "./utils.js";
...
const blockedBookingMsg = document.getElementById("blockedBookingMsg");
...
async function loadConfig() {
  try {
    ...
    snap.forEach((d) => {
      if (d.id === "business") businessConfig = mergeBusinessConfig(d.data());
    });
  } catch (e) {
...
bookingForm.addEventListener("submit", async (e) => {
  ...
  blockedBookingMsg.hidden = true;

  if (businessConfig.penalty?.enabled) {
    try {
      const blockedUntil = await getBlockedUntil(clientPhone);
      if (blockedUntil) {
        blockedBookingMsg.textContent = `No es posible agendar una cita nueva hasta el ${formatFriendlyDate(blockedUntil)} debido a cancelaciones tardías repetidas. Si crees que esto es un error, contáctanos directamente.`;
        blockedBookingMsg.hidden = false;
        submitBtn.disabled = false;
        submitBtn.textContent = "Confirmar cita";
        return;
      }
    } catch (e) {
      console.warn("No se pudo verificar el estado de penalización, se continúa con la reserva.", e);
    }
  }
...
// Devuelve la fecha (YYYY-MM-DD) hasta la que sigue bloqueada esta clienta,
// o null si no tiene ningún bloqueo vigente.
async function getBlockedUntil(phoneRaw) {
  const phone = normalizePhone(phoneRaw);
  if (!phone) return null;
  const snap = await getDoc(doc(db, "clientPenalties", phone));
  if (!snap.exists()) return null;
  const data = snap.data();
  if (data.blockedUntil && data.blockedUntil >= todayKey()) return data.blockedUntil;
  return null;
}

// Registra una cancelación tardía (o reinicia la racha si la cancelación se
// hizo con suficiente anticipación) para el teléfono dado. Si al sumar esta
// cancelación se llega al máximo configurado, aplica el bloqueo.
async function registerCancellation(phoneRaw, clientName, wasLate) {
  const penalty = businessConfig.penalty;
  ...
  try {
    const ref = doc(db, "clientPenalties", phone);
    const snap = await getDoc(ref);
    const existing = snap.exists() ? snap.data() : {};

    if (!wasLate) {
      // Una cancelación con suficiente anticipación corta la racha de
      // cancelaciones tardías seguidas (pero no levanta un bloqueo ya activo).
      await setDoc(ref, {
        phone,
        lastClientName: clientName || existing.lastClientName || "",
        lateCancelStreak: 0,
    ...
    const newStreak = (existing.lateCancelStreak || 0) + 1;
    const reachedLimit = newStreak >= penalty.maxLateCancels;

    await setDoc(ref, {
      phone,
      lastClientName: clientName || existing.lastClientName || "",
      lateCancelStreak: reachedLimit ? 0 : newStreak,
      blockedUntil: reachedLimit ? addDaysToKey(todayKey(), penalty.blockDays) : (existing.blockedUntil || null),
      updatedAt: serverTimestamp()
    });
  } catch (e) {
    // No queremos que un fallo aquí impida completar la cancelación misma.
    console.warn("No se pudo registrar la penalización por cancelación.", e);
  }
}
...
confirmCancelBtn.addEventListener("click", async () => {
  ...
  try {
    const wasLate = businessConfig.penalty?.enabled
      && hoursUntil(foundAppointment.date, foundAppointment.time) <= businessConfig.penalty.lateWindowHours;
    ...
    await registerCancellation(foundAppointment.clientPhone, foundAppointment.clientName, wasLate);
    cancelResult.hidden = true;
