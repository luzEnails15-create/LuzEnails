...
import {
  ...
  formatMoney, formatDuration, formatFriendlyDate,
...
function renderTimeSlots() {
  ...
  slotGridEl.innerHTML = free
    .map((t) => `<button class="slot-ticket" data-time="${t}">${t}</button>`)
    .join("");
...
function showConfirmation(clientName, appointmentId) {
  ...
  confirmDetails.innerHTML = `
    ...
    <div class="row"><span>Servicio</span><span>${escapeHtml(selectedService.name)}</span></div>
    <div class="row"><span>Fecha</span><span>${formatFriendlyDate(selectedDateStr)}</span></div>
    <div class="row"><span>Hora</span><span>${selectedSlot}</span></div>
  `;
...
cancelLookupForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    cancelResultDetails.innerHTML = `
      ...
      <div class="row"><span>Hora</span><span>${appt.time}</span></div>
    `;
...
confirmCancelBtn.addEventListener("click", async () => {
  ...
  if (!confirm(`¿Seguro que quieres cancelar tu cita del ${formatFriendlyDate(foundAppointment.date)} a las ${foundAppointment.time}?`)) return;
